...
BEGIN_PROVIDER [ logical, ao_two_e_integrals_bsplines_in_map ]
  implicit none
  use f77_zmq
  use map_module
  BEGIN_DOC
...
!  endif
  ...
  call new_parallel_job(zmq_to_qp_run_socket,zmq_socket_pull,'ao_integrals_bsplines')

  character(len=:), allocatable :: task
  allocate(character(len=ao_dim*12) :: task)
  write(fmt,*) '(', ao_dim, '(I5,X,I5,''|''))'
  do l=1,ao_dim
  ...
  !$OMP PARALLEL DEFAULT(shared) private(i) num_threads(nproc+1)
      ...
      if (i==0) then
        call ao_two_e_integrals_bsplines_in_map_collector(zmq_socket_pull)
      else
        call ao_two_e_integrals_bsplines_in_map_slave_inproc(i)
      endif
  !$OMP END PARALLEL

  call end_parallel_job(zmq_to_qp_run_socket, zmq_socket_pull, 'ao_integrals_bsplines')
  ...
  integer(map_size_kind)         :: get_ao_bsplines_map_size, ao_bsplines_map_size
  ao_bsplines_map_size = get_ao_bsplines_map_size()

  print*, 'AO ERF integrals provided:'
  print*, ' Size of AO ERF map :         ', map_mb(ao_integrals_bsplines_map) ,'MB'
  print*, ' Number of AO ERF integrals :', ao_bsplines_map_size
